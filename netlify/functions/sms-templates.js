...
/**
 * SMS Templates API
 * Manages SMS templates for follow-up campaigns
 */
...
async function getTemplates(event) {
  ...
  try {
    ...
    if (filters.search) {
      sql += ` AND (name ILIKE $${paramIndex} OR description ILIKE $${paramIndex} OR content ILIKE $${paramIndex})`;
      params.push(`%${filters.search}%`);
      paramIndex++;
    }

    // Add sorting
    const validSortFields = ['name', 'template_type', 'usage_count', 'character_count', 'created_at', 'updated_at'];
    ...
    let countSql = 'SELECT COUNT(*) as total FROM sms_templates WHERE 1=1';
    ...
    if (filters.search) {
      countSql += ` AND (name ILIKE $${countParamIndex} OR description ILIKE $${countParamIndex} OR content ILIKE $${countParamIndex})`;
      countParamIndex++;
    }
    ...
    return errorHandler.serverError('Failed to get SMS templates', error);
  }
}
...
async function createTemplate(event) {
  ...
  if (!name || !content) {
    return errorHandler.validationError('Missing required fields', {
      ...
      content: !content ? 'SMS content is required' : null
    });
  }

  // Validate content length (SMS limit)
  if (content.length > 160) {
    return errorHandler.validationError('SMS content too long', {
      content: `SMS content must be 160 characters or less. Current length: ${content.length}`
    });
  }
  ...
  try {
    const existingSql = 'SELECT id FROM sms_templates WHERE name = $1';
  ...
  try {
    const sql = `
      INSERT INTO sms_templates (
        name, description, content, template_type, is_active,
        variables, test_phone, tags, metadata, created_by
    ...
    return errorHandler.serverError('Failed to create SMS template', error);
  }
}
...
async function getTemplate(event, templateId) {
  try {
    const sql = 'SELECT * FROM sms_templates WHERE id = $1';
    ...
    const usageSql = `
      ...
      FROM followups
      WHERE sms_template = $1 AND status = 'sent'
    `;
    ...
    return errorHandler.serverError('Failed to get SMS template', error);
  }
}
...
async function updateTemplate(event, templateId) {
  ...
  delete updateData.character_count; // This is computed
  delete updateData.message_count; // This is computed
  ...
  if (updateData.content && updateData.content.length > 160) {
    return errorHandler.validationError('SMS content too long', {
      content: `SMS content must be 160 characters or less. Current length: ${updateData.content.length}`
    });
  }
  ...
  if (updateData.name) {
    try {
      const existingSql = 'SELECT id FROM sms_templates WHERE name = $1 AND id != $2';
  ...
  try {
    const allowedFields = [
      'name', 'description', 'content', 'template_type', 'is_active',
      'variables', 'test_phone', 'tags', 'metadata', 'updated_by'
    ];
    ...
    return errorHandler.serverError('Failed to update SMS template', error);
  }
}
...
async function deleteTemplate(event, templateId) {
  try {
    ...
    const rulesSql = 'SELECT COUNT(*) as rules_count FROM followup_rules WHERE sms_template = $1';
    ...
    const followupsSql = 'SELECT COUNT(*) as followups_count FROM followups WHERE sms_template = $1';
    ...
    const sql = 'DELETE FROM sms_templates WHERE id = $1 RETURNING *';
    const result = await DatabaseService.query(sql, [templateId]);
    ...
    return errorHandler.serverError('Failed to delete SMS template', error);
  }
}
...
async function duplicateTemplate(event, templateId) {
  ...
  try {
    ...
    const originalSql = 'SELECT * FROM sms_templates WHERE id = $1';
    ...
    return errorHandler.serverError('Failed to duplicate SMS template', error);
  }
}
...
async function testTemplate(event, templateId) {
  ...
  try {
    // Get the template
    const templateSql = 'SELECT * FROM sms_templates WHERE id = $1';
    ...
    // Send test SMS if test_phone is configured
    let smsSent = false;
    if (template.test_phone && testData.send_test) {
      // Here you would integrate with your SMS service to send the test
      console.log(`Test SMS would be sent to: ${template.test_phone}`);
      console.log(`Content: ${personalizedContent}`);
      console.log(`Character count: ${personalizedContent.length}`);
      console.log(`Message count: ${Math.ceil(personalizedContent.length / 153)}`);
      smsSent = true; // In real implementation, check if SMS was sent successfully
    }
    ...
    return errorHandler.createSuccessResponse({
      ...
      character_count: personalizedContent.length,
      message_count: Math.ceil(personalizedContent.length / 153),
      sms_sent: smsSent,
      test_phone: template.test_phone
    });

  } catch (error) {
    console.error('Error testing SMS template:', error);
    return errorHandler.serverError('Failed to test SMS template', error);
  }
}
...
async function getTemplateUsage(event, templateId) {
  ...
  try {
    const usageSql = `
      ...
      FROM followups
      WHERE sms_template = (SELECT name FROM sms_templates WHERE id = $1)
    ...
    const campaignsSql = `
      ...
      WHERE fr.sms_template = (SELECT name FROM sms_templates WHERE id = $1)
    ...
    console.error('Error getting SMS template usage:', error);
    return errorHandler.serverError('Failed to get SMS template usage', error);
  }
}
